...
public class EL2 {
    ...
    public static double lossFunction(DEFAULT_LOSS_FUNCTIONS lossFunction, int trueClass, double[] predictedValues) throws Exception {
        switch (lossFunction) {
            ...
            case CATEGORICAL_CROSS_ENTROPY_LOSS_FUNCTION -> { return categoricalCrossEntropy(trueClass, predictedValues); }
            default -> {
    ...
    // for iterative
    private static double categoricalCrossEntropy(int trueValue, double[] predictedProbability){
        return -Math.log(predictedProbability[trueValue]);
    }
    ...
    private static double negativeLogLikelihood(int trueClass, double[] predictedValues) {
        ...
        predictedProbability = Math.max(predictedValues[trueClass], epsilon);
